...
int main(int argc, char **argv)
{
  ...
  srv.request.spaceSize.push_back(500);
  srv.request.spaceSize.push_back(500);
  srv.request.spaceSize.push_back(300);
  srv.request.spaceCenter.push_back(0);
  srv.request.spaceCenter.push_back(-100);
...
//   reqPose.position.x = 500.0;
//   reqPose.position.y = 500.0;
...
//   srv.request.requiredPoses.push_back(reqPose);
  ...
  srv.request.minDist = 30.0;
  srv.request.maxDist = 100.0;
  srv.request.numIterations = 30;

  /* read STL file and publish to rviz */
  std::vector<nav_msgs::Path> * mesh = readSTLfile(ros::package::getPath("request")+"/meshes/planenew_simplification.stl");
